      ...
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError *error))completionHandler
{
    ...
    unsigned char *bufferCache=malloc(20*1024*1024),*bufferCachePtr = bufferCache;
    memset(bufferCache,0,20*1024*1024);;
    unsigned char *bufferUn=malloc(LZFSE_RATE*20*1024*1024);
    memset(bufferUn,0,LZFSE_RATE*20*1024*1024);
                ...
                while (fp) {
                    ...
                    if (readBytes > 0) {
                        if(fileInfo.compression_method==99){
                            //cache
                            memcpy(bufferCachePtr,buffer,readBytes);
                            bufferCachePtr+=readBytes;
                        }else
                            fwrite(buffer, readBytes, 1, fp );
                    } else {
                        if(fileInfo.compression_method==99){
                            //flush
                            int in_size = bufferCachePtr - bufferCache;
                            int out_size = lzfse_decode_buffer(bufferUn, LZFSE_RATE*in_size, bufferCache, in_size, aux);
                            assert(out_size);
                            fwrite(bufferUn, out_size, 1, fp );
                            //reclear
                            memset(bufferCache,0,20*1024*1024);;
                            memset(bufferUn,0,LZFSE_RATE*20*1024*1024);
                            bufferCachePtr = bufferCache;
                        }
                        break;
                    }
                }
